...
export function ArticleCard({ post, stats, featured = false }) {
  ...
  return <a className={`article-card${post.image ? " has-image" : ""}${featured ? " is-featured" : ""}`} href={contentRoute("post", post)}><ArticleCover post={post} className="article-card-media" emptyClassName="article-card-media--placeholder" imageClassName="article-card-media-image" /><div className="article-card-copy"><div className="article-card-kickers"><span className="category">{post.category}</span></div><h2>{post.title}</h2>{(post.series || post.tags?.length > 0) && <div className="article-card-tags">{post.series && <span className="series-kicker"><FolderOpen size={13} />{post.series}</span>}{post.tags?.slice(0, 3).map((tag) => <span key={tag}>#{tag}</span>)}</div>}{summary && <div className="article-card-excerpt"><span>{post.excerpt ? "摘要" : "正文预览"}</span><p>{summary}</p></div>}<PostMeta post={post} showTags={false} stats={stats} /></div></a>;
}
