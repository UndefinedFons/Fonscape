...
export function MusicReviewCard({ entry, section, stats, imageLoading = "lazy" }) {
  ...
  return <a className={`article-card music-review-card${entry.image ? " has-image" : ""}`} href={contentRoute("music", { ...entry, section })}><div className="music-review-layout">{entry.image && <span className="article-card-media music-review-media"><img className="article-card-media-image" {...responsiveImage} alt="" loading={imageLoading} decoding="async" fetchPriority={imageLoading === "eager" ? "high" : "low"} /></span>}<div className="article-card-copy"><span className="category">标题</span><h2>{entry.title}</h2>{summary && <div className="article-card-excerpt"><span>{entry.excerpt ? "摘要" : "正文预览"}</span><p>{summary}</p></div>}<div className="music-review-credits"><CreditIcon size={16} weight="duotone" /><span><strong>{entry.sourceTitle || entry.title}</strong>{(entry.sourceMeta || entry.kind) && <em>{entry.sourceMeta || entry.kind}</em>}</span></div><PostMeta post={entry} showTags={false} stats={stats} /></div></div></a>;
}
