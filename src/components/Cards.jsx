...
export function ArticleCard({ post, stats, featured = false, imageLoading = "lazy" }) {
  const summary = post.excerpt || getPostFirstParagraph(post);
  return <a className={`article-card${post.image ? " has-image" : ""}${featured ? " is-featured" : ""}`} href={contentRoute("post", post)}><ArticleCover post={post} className="article-card-media" emptyClassName="article-card-media--placeholder" imageClassName="article-card-media-image" loading={imageLoading} fetchPriority={imageLoading === "eager" ? "high" : undefined} /><div className="article-card-copy"><div className="article-card-kickers"><span className="category">{post.category}</span></div><h2>{post.title}</h2>{(post.series || post.tags?.length > 0) && <div className="article-card-tags">{post.series && <span className="series-kicker"><FolderOpen size={13} />{post.series}</span>}{post.tags?.slice(0, 3).map((tag) => <span key={tag}>#{tag}</span>)}</div>}{summary && <div className="article-card-excerpt"><span>{post.excerpt ? "摘要" : "正文预览"}</span><p>{summary}</p></div>}<PostMeta post={post} showTags={false} stats={stats} /></div></a>;
}

export function MusicReviewCard({ entry, section, stats, imageLoading = "lazy" }) {
  ...
  const responsiveImage = useResponsiveImage(imageSource, "188px");
  return <a className={`article-card music-review-card${entry.image ? " has-image" : ""}`} href={contentRoute("music", { ...entry, section })}><div className="music-review-layout">{entry.image && <span className="article-card-media music-review-media"><img className="article-card-media-image" {...responsiveImage} alt="" loading={imageLoading} decoding="async" fetchPriority={imageLoading === "eager" ? "high" : "low"} /></span>}<div className="article-card-copy"><span className="category">标题</span><h2>{entry.title}</h2>{summary && <div className="article-card-excerpt"><span>{entry.excerpt ? "摘要" : "正文预览"}</span><p>{summary}</p></div>}<div className="music-review-credits"><CreditIcon size={16} weight="duotone" /><span><strong>{entry.sourceTitle || entry.title}</strong>{(entry.sourceMeta || entry.kind) && <em>{entry.sourceMeta || entry.kind}</em>}</span></div><div className="post-meta"><span><TextAa size={16} />{getPostWordCount(entry)} 字</span><span><CalendarBlank size={16} />{formatContentDate(entry.date)}</span><span><Eye size={16} />{stats?.views || 0}</span><span><ChatCircleDots size={16} />{stats?.comments || 0}</span></div></div></div></a>;
}
