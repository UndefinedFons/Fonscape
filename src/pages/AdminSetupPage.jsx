...
export function AdminSetupPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm({ token: "", username: "", password: "" });
  ...
  if (state.loading) {
    ...
    content = <><header className="admin-setup-heading"><small>ONE-TIME SETUP</small><h1>创建管理员</h1><p>仅首次部署需要完成一次。请输入部署时由你设置的管理员令牌，再创建登录账户。</p></header><form className="community-form admin-setup-form" onSubmit={submit}><label htmlFor="admin-setup-token"><span>管理员令牌</span><span className="community-input"><Key size={18} aria-hidden="true" /><input id="admin-setup-token" name="token" type="text" value={form.token} onChange={update("token")} autoComplete="off" spellCheck="false" required placeholder="输入部署时设置的令牌" /></span><small>令牌保持可见，方便核对；此处不会预填任何内容。</small></label><label htmlFor="admin-setup-username"><span>账户名</span><span className="community-input"><At size={18} aria-hidden="true" /><input id="admin-setup-username" name="username" value={form.username} onChange={update("username")} autoComplete="username" minLength="3" maxLength="20" pattern="[A-Za-z0-9]{3,20}" required placeholder="3–20 位英文字母或数字" /></span></label><label htmlFor="admin-setup-password"><span>密码</span><span className="community-input"><LockKey size={18} aria-hidden="true" /><input id="admin-setup-password" name="password" type="password" value={form.password} onChange={update("password")} autoComplete="new-password" minLength="6" maxLength="20" pattern="[A-Za-z0-9]{6,20}" required placeholder="6–20 位英文字母或数字" /></span></label>{state.error && <p className="community-form-error" role="alert"><WarningCircle size={17} aria-hidden="true" />{state.error}</p>}<button className="community-primary-button" type="submit" disabled={busy}>{busy ? "正在创建…" : "创建管理员"}<ArrowRight size={17} /></button></form></>;
  }
