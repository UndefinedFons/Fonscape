...
export function ArticlePage({ slug, stats, onView }) {
  ...
  return <main className="article-page material-panel page-width"><button className="back-button" onClick={() => history.length > 1 ? history.back() : go("/posts")}><ArrowLeft size={17} />返回</button><article className={`article-detail article-detail--${coverMode}`}>
    <div className={`article-intro article-intro--${coverMode}`}>
      ...
      {showDetailCover && <ZoomableImage src={post.image} alt={`${post.title}的文章封面`} showLightboxCaption={false} sizes={detailImageSizes} className="article-cover" triggerClassName="article-cover-frame" style={{ objectPosition: post.coverPosition || "center" }} loading="eager" />}
    </div>
