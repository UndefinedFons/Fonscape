...
export function PoemsPage({ stats, onStatsTargets }) {
  ...
  const pageStatsKey = pagination.pageItems.map((poem) => poem.slug).join("|");
  useEffect(() => { onStatsTargets(pagination.pageItems.map((poem) => ({ type: "poem", slug: poem.slug }))); }, [onStatsTargets, pageStatsKey]);
