...
export function MusicPage({ stats, onStatsTargets }) {
  const collection = useCollectionIndex("music", !collection.loading && !collection.error);
  const allMusicReviews = collection.items;
  ...
  const pageContent = useCollectionPage("music", pagination.pageItems);
  ...
  return <section className="music-library page-width">
      ...
      <div ref={pagination.topRef} className={`music-panel paginated-view${pagination.leaving ? " is-leaving" : ""}`} role="tabpanel" key={section}><CollectionLoadStatus {...pageContent} />{collection.loading || (pageContent.loading && !pageContent.items.length && !pageContent.error) ? <div className="music-empty" role="status">正在加载…</div> : collection.error ? <CollectionLoadStatus {...collection} /> : pageContent.error && !pageContent.items.length ? null : activeEntries.length ? <div className="music-review-grid">{pageContent.items.map((entry, index) => <MusicReviewCard entry={entry} section={section} stats={stats[`${section}/${entry.slug}`]} imageLoading={index === 0 ? "eager" : "lazy"} key={entry.slug} />)}</div> : <div className="music-empty"><ActiveSectionIcon size={36} weight="duotone" /><h2>{section === "songs" ? "暂无歌曲" : section === "artists" ? "暂无音乐人" : "暂无专辑"}</h2></div>}</div><Pagination page={pagination.page} totalPages={pagination.totalPages} onChange={pagination.changePage} />
    </section>;
}
