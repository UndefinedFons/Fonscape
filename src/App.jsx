...
function DetailRouteFallback({ route }) {
  if (route.startsWith("/poem/")) {
    return <main className="poem-page page-width detail-route-pending" aria-label="正在载入小诗" aria-busy="true"><article aria-hidden="true" /></main>;
  }
  const music = route.startsWith("/music/");
  return <main className={`article-page${music ? " music-detail-page" : ""} material-panel page-width detail-route-pending`} aria-label={music ? "正在载入音乐手记" : "正在载入文章"} aria-busy="true" />;
}
...
export function App() {
  ...
  return <div className={themeChanging ? "app-shell theme-changing" : "app-shell"} style={{
    ...
    <div className={isDetailRoute ? "route-view route-view--detail" : "route-view"} key={route}><Suspense fallback={isDetailRoute ? <DetailRouteFallback route={route} /> : null}><RouteContent route={route} routeQuery={routeQuery} stats={contentStats} onView={recordContentView} onOutline={setActivePostOutline} onRequestStats={requestContentStats} isRetiredAdminRoute={isRetiredAdmin} routeEnabled={routeEnabled} /></Suspense></div>{!isSetupRoute && <><Footer /><DialogFrame kind="search" open={searchOpen} onClose={() => setSearchOpen(false)} label="搜索博客内容">{(requestClose, markLoadingShown) => <Suspense fallback={<DialogSkeleton kind="search" onShown={markLoadingShown} />}><SearchDialog onClose={requestClose} /></Suspense>}</DialogFrame><DialogFrame kind="settings" open={settingsOpen} onClose={() => setSettingsOpen(false)} label="显示设置">{(requestClose, markLoadingShown) => <Suspense fallback={<DialogSkeleton kind="settings" onShown={markLoadingShown} />}><SettingsDialog glassEnabled={glassEnabled} onGlassChange={handleGlassChange} onClose={requestClose} /></Suspense>}</DialogFrame><DialogFrame kind="account" open={accountOpen} onClose={closeAccount} label={viewer ? "个人中心" : "账户登录"}>{(requestClose, markLoadingShown) => <Suspense fallback={<DialogSkeleton kind="account" onShown={markLoadingShown} />}><AccountDialog onClose={requestClose} /></Suspense>}</DialogFrame>{accountNotice && <aside className="community-account-notice" role="alert"><div><strong>账户通知</strong><p>{accountNotice}</p></div><button type="button" onClick={() => dismissAccountNotice()}>知道了</button></aside>}</>}
  </div>;
}
