...
export function CommentsSection(props) {
  return siteConfig.showCommunity ? <EnabledCommentsSection {...props} /> : null;
}

function EnabledCommentsSection({ targetType, slug, onStatsChange }) {
  const { state, threads, locatedCommentId, pageTopRef, replyProps, changePage, refreshComments, retry } = useCommentsSection({ targetType, slug, onStatsChange });
