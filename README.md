...
- **沉浸式长文阅读**：支持 GFM Markdown、代码高亮、表格、提示块、KaTeX 数学公式与 Mermaid 图表，以及自动编号目录、阅读进度、图片灯箱、系列导航和可选文章配乐。公式与图表按需加载，图表颜色随主题调整。
- **连贯的浏览体验**：详情页可返回站内来源并恢复列表位置，页面标题随当前内容更新，滚动与灯箱交互尊重系统的减少动态效果设置。
- **可选的内容板块**：文章作为基础板块；小诗与音乐可分别开启，导航、首页、搜索、路由与资源预加载统一遵循开关。
- **评论消息追溯**：“我的消息”“收到评论”和“收到回复”保留内容来源与回复上下文；发表新评论或回复后也会自动定位，支持跨页查找和失败重试。
- **文章订阅**：配置正式站点地址后，构建自动生成 RSS，读者可通过订阅工具获取新文章。
