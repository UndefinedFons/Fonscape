<p align="center">
  Fonscape 是一款面向个人写作者的 React 博客主题，提供可定制界面、Markdown 内容管理、长文阅读、可选社区功能，以及 Cloudflare 与 Vercel 一键部署。
</p>
...
<p align="center">
  <a href="https://fonstage.space/"><img src="./.github/assets/fonstage-home.webp" width="100%" alt="Fonstage 的 Fonscape 桌面首页，展示头图导航、作者卡与置顶文章"></a>
</p>
...
构建时，主题自动扫描 Markdown 与 Frontmatter，生成前端和 API 共用的内容目标，并把列表、搜索索引与详情正文拆成按需加载的固定分块；各分块携带自身的响应式图片描述，图片无需等待全站清单即可开始请求。部署后，Cloudflare Workers + D1 或 Vercel + Turso 负责各自的动态数据。仓库内容始终是发布内容的唯一来源。

## 核心体验

- **一致的视觉系统**：页面头图、明暗主题、可切换的磨砂材质与响应式布局保持统一，并支持站点级自定义。
- **沉浸式长文阅读**：支持 Markdown、代码高亮、提示块、数学公式与图表，配备文章目录、阅读进度、图片灯箱、系列导航和[文章配乐](./GUIDE.md#文章配乐)；配乐默认使用 Meting 单曲 URL，也可改用本地媒体。
- **可选的内容与社区板块**：以文章为基础，可按需开启小诗与音乐手记；也可关闭账户、评论与友链，保留纯静态阅读体验。
- **评论消息追溯**：启用社区功能时，可集中查看收到的评论与回复，沿消息回到原文和讨论上下文。
- **文章订阅**：提供 RSS 订阅，让读者通过订阅工具获取新文章。
- **简洁的写作与部署流程**：使用 Markdown 与 Git 管理内容，支持 Cloudflare、Vercel 一键部署和保护个人内容的主题升级。
...
站点资料、作者信息、页面文案、板块显示开关和头图从 `fonscape.config.js` 读取。小诗与音乐默认隐藏，账户、评论与友链默认开启；可在仓库根目录配置：
...
`showCommunity` 用于关闭社区功能；各开关的完整行为见 [使用指南](./GUIDE.md#账户评论与友链显示开关)。
...
开发新板块、设置项或图片入口时，请同时遵守使用指南中的维护与功能扩展约束，让功能接入统一内容分块、性能预算与共享组件体系，避免形成平行架构和重复逻辑。新增或调整测试时，遵循 [测试规则](./GUIDE.md#测试)。
