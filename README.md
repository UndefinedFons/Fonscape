...
文章分类的名称、数量和顺序也可在同一文件中自定义，例如：

```js
postCategories: ["记录", "摄影", "旅行"],
```

主题会自动添加“全部”，各选项均分现有分类栏；省略此配置时保留默认六类。文章 Frontmatter 的 `category` 填写对应分类名即可，例如 `category: "摄影"`；修改配置不会自动改写已有文章的分类。更多规则见 [文章分类](./GUIDE.md#文章分类)。
