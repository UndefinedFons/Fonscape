...
支持 `NOTE`、`TIP`、`IMPORTANT`、`WARNING` 与 `CAUTION` 五种标记，显示对应的轻量颜色、图标和标题。普通引用沿用同款布局，保留灰色引用图标与浅灰背景，并随明暗主题调整；已有 Markdown 引用无需修改，升级后会自动使用新样式。
...
含公式的文章会按需加载 KaTeX 与对应样式；没有公式的文章不会请求这些资源。无时区的 Frontmatter 日期在 RSS 中按 UTC 解释；需要表达其他时区时请填写带时区的 ISO 日期。
...
图表以自然尺寸居中显示，不会被拉伸填满正文。图表运行时按需加载 Mermaid，使用严格安全模式；如果语法错误或运行时加载失败，正文会保留并显示原始代码。图表容器在窄屏下可以横向滚动，不会撑开页面。Mermaid 解析器和布局引擎使用独立的 160 KiB gzip 单包上限；其他动态分块仍为 96 KiB，构建检查同时禁止 Mermaid 进入首页静态依赖。
...
配置 `siteUrl` 后，构建会从 `src/content/posts/*.md` 读取文章 Frontmatter，使用与内容清单相同的解析和时间排序规则生成 `/feed.xml`。每个条目的链接和 `guid` 都是基于 `siteUrl` 与文章 slug 的稳定绝对地址，发布时间使用文章的 `date`。`siteUrl` 为空时不会生成 feed，也不会写入示例域名；无效的 URL 会使构建直接报错。

站点同时提供公开的 `/robots.txt`，允许正常页面抓取并禁止 `/api/` 与 `/admin` 路径。主题不生成 sitemap。
