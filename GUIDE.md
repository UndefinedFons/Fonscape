...
| `postCategories` | 文章页类型标签；省略时使用 `随笔`、`小说`、`评谈`、`记录`、`笔记`、`指南` |
| `home.*` | 首页头图短标题、主标题与简介 |
...
`postCategories` 位于 `fonscape.config.js` 的根级 `siteConfig` 对象中，用于配置文章页的类型标签。主题会自动在最前面加入 `全部`；该名称由主题保留，不能作为自定义分类。分类名称会去除首尾空白、忽略空字符串并去重，文章内容中的其他分类不会自动生成新的标签：

```js
postCategories: ["随笔", "小说", "评谈", "记录", "笔记", "指南"],
```

例如，配置 `postCategories: ["摄影", "旅行"]` 后，文章 Frontmatter 可写 `category: "摄影"`：名称必须与配置完全匹配；修改配置不会改写已有文章的 `category`。

省略 `postCategories` 时使用上述六个默认分类；设置为空数组 `[]` 时只显示 `全部`。自定义分类可以使用任意非空名称，未列入配置的文章仍会在 `全部` 中显示。
...
- 评论频率保护保留账户短周期与每日上限、IP 短周期上限、全站每小时与每日上限；单个内容页面不再额外叠加小时窗口。管理员只绕过已登录账户操作的频率限制，重复评论检测、原子写入和所有容量熔断仍然生效。
