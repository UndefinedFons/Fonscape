...
| `siteUrl` | 站点公开的 `http(s)` 根地址，例如 `https://example.com`，不含子目录；用于生成 RSS 与站点地图，留空则不生成这两项文件 |
...
两个开关位于仓库根目录 `fonscape.config.js` 的根级 `siteConfig` 对象中，与 `language`、`title`、`description` 同级。它们统一控制导航、首页、统计、搜索、列表、详情与资源预加载。关闭板块后，访问对应列表或详情地址会返回首页：
...
自定义图片放在 `public/assets/`，配置路径从网站根目录开始写，例如 `public/assets/home.jpg` 对应 `"/assets/home.jpg"`。文件名大小写必须与仓库中的文件一致。每个位置只需提供一张质量足够的原图；Fonscape 会按头像、卡片、正文和头图等实际渲染角色，为每张原图最多生成 small、large 两个实际响应式派生文件，并可把一个低清占位图作为内联数据随内容提供。派生图优先使用高保真 WebP 编码，以 1 MiB 为质量收敛目标，并以 2 MiB 为硬上限；复杂图片只在超出目标时逐级调整编码，超过硬上限会让构建失败并提示更换合适的原图。如果高清派生图不能明显减小传输体积，主题会把原图直接保留为高清候选。普通页面直接请求适合视口和屏幕像素密度的候选，图片元素首次渲染即有可用 `src`；正文灯箱只在用户打开后请求原图。原图始终作为生成输入和必要兜底，派生图位于忽略提交的构建目录，不需要手工维护，也不会替换原图。
...
- 图片与图片标题
- GitHub 风格提示块、KaTeX 公式与 Mermaid 图表
...
支持 `NOTE`、`TIP`、`IMPORTANT`、`WARNING` 与 `CAUTION` 五种标记，显示对应的轻量颜色、图标和标题。普通引用沿用同款布局，保留灰色引用图标与浅灰背景，并随明暗主题调整。

#### 数学公式

使用单个 `$` 包围行内公式，或使用一对 `$$` 编写块级公式：
...
公式由 KaTeX 渲染，相关资源按需加载。

#### Mermaid 图表

将 Mermaid 定义写在语言标记为 `mermaid` 的 fenced code block 中：
...
图表使用主题配色与圆角，以自然尺寸居中显示；窄屏下可在图表内横向滚动。Mermaid 按需加载，语法或加载出错时显示原始代码。
...
生成器只把首页需要的少量摘要和各板块计数放进主题入口；列表、筛选与搜索索引按固定大小分块，详情正文按单条内容独立加载。各内容分块只携带自身图片的 small/large 描述和可选内联 LQIP，浏览器无需先下载全站图片清单即可请求真实图片。性能预算约束入口主题包和每个分块，不约束全站内容总数；内容增加时增长的是分块数量，不会在达到某个总条目数后阻止继续发布。分块规则以 `scripts/generate-content-targets.mjs` 中的 `CONTENT_PAGE_CHUNK_SIZE` 和 `CONTENT_INDEX_CHUNK_SIZE` 为准。
...
配置 `siteUrl` 后，构建会从 `src/content/posts/*.md` 读取文章 Frontmatter，使用与内容清单相同的解析和时间排序规则生成 `/feed.xml`。每个条目的链接和 `guid` 都是基于 `siteUrl` 与文章 slug 的稳定绝对地址，发布时间使用文章的 `date`。无时区日期按 UTC 解释；其他时区请使用带时区的 ISO 日期。`siteUrl` 为空时不会生成 feed，也不会写入示例域名；无效的 URL 会使构建直接报错。

站内页面使用 `/posts`、`/post/文章-slug` 等路径，可直接打开或刷新。RSS 中的文章链接使用同样的路径。

构建同时生成 `/sitemap.xml`，收录公开页面与内容详情；小诗和音乐仅在对应板块开启时收录。管理页面和 API 不进入站点地图。`siteUrl` 留空时不生成站点地图。

站点同时提供公开的 `/robots.txt`，允许正常页面抓取并禁止 `/api/` 与 `/admin` 路径。
...
- 新图片入口必须登记到现有角色化响应式图片生成器。单张原图最多生成 small、large 两个实际派生文件，可附带一个内联 LQIP；不要为某个新页面另写一套图片副本或全量运行时清单，也不要把内容总量塞回入口 JS、CSS 或 HTML 预算。
...
- 交互性能以冷缓存首次访问后 0.5–1 秒内立即操作为基线。一级导航应同步提交 URL、当前导航项、目标 Hero/LQIP 与页面 Shell；常用弹窗应立即显示最终容器，加载状态只在容器内部出现，并与真实内容平滑交接。预取按当前页面、明确意图、空闲任务分级，空闲任务不得抢占当前页面。
- 性能优化不得改变既有前端动画的状态、时长、缓动、触发顺序或 `prefers-reduced-motion` 行为。需求没有明确指定动画变化时，不得删除、缩短、跳过、替换或重新设计动画效果。
...
Mermaid 解析器和布局引擎使用独立的 160 KiB gzip 单包上限；其他动态分块仍为 96 KiB，构建检查同时禁止 Mermaid 进入首页静态依赖。

### 测试

测试面向主题的公开契约：内容解析、路由、权限、数据库一致性、部署适配、可访问性与性能预算。单元和集成测试使用 Node.js 内置测试运行器，浏览器中的关键评论路径使用 Playwright。
...
行为保护用户可见契约、安全边界或数据完整性，或高风险回归难以通过类型检查和 Lint 发现时，才添加长期测试；属于同一契约的行为优先扩展现有测试文件。新功能覆盖主成功路径和主要失败路径，缺陷修复只添加修复前失败、修复后通过的最小测试，能用一个参数化用例表达时不为每个分支复制独立用例。

一条测试只有在被保护的行为失效时失败，才算有效。新增或改写用例后，用故障注入或等价方法确认这一点，再恢复代码；排序、去重和筛选使用打乱顺序、含重复值的输入，让“原样返回”无法通过。测试断言运行时可观察的结果而非实现文本，读取源码的测试必须有明确的高风险理由；同一行为只在最合适的一层保护，不在单元、集成和 E2E 中重复断言。优先复用固定数据和 helper，保持一个测试对应一个契约，名称描述当前行为而不保留历史版本号，并让失败信息直接指向问题。涉及交互和动画时保持可访问性与 `prefers-reduced-motion` 行为，依赖时间的测试使用注入的时钟。

被测功能消失、契约已在更合适的层级得到保护、兼容承诺到期，或测试只固定源码文本、内部 DOM 形状和排版细节时，随改动一起删除；类型检查、Schema 校验或运行时校验已提供同等保护的用例也不必保留。不要因为某个测试曾经发现过缺陷就永久保留，保留条件是缺陷仍可能发生且该测试是最清楚的保护方式。功能删除时同步删除对应测试、固定数据和 helper。覆盖率与测试数量仅作参考，不作为目标；常规本地测试在没有扩大覆盖范围的情况下明显变慢时，检查重复的迁移初始化、浏览器重测试和同一集成路径的重复用例。
