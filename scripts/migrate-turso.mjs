...
async function appliedMigrations(client, createTable) {
  if (createTable) {
    await client.execute(`CREATE TABLE IF NOT EXISTS ${migrationTable} (
  ...
  try {
    const result = await client.execute(`SELECT name, checksum FROM ${migrationTable} ORDER BY name`);
