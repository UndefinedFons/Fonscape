...
- 默认配置与文档进一步注明自定义图片在配置中应使用带引号的 `"/assets/..."` 网站路径，避免把 `public/assets/...` 当成 JavaScript 值导致部署构建失败。
...
- 评论操作按“回复、删除、复制友链 JSON”排列，让仅管理员可见的友链处理动作保持在末尾。
- 首次管理员设置补充昵称输入，并为密码框增加显示与隐藏开关。
