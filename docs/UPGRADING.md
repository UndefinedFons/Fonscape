...
数据库迁移属于只可追加的执行历史：Updater 会添加目标版本的新迁移，但不会删除站点已有的迁移；相同文件名若内容不同会停止并要求人工核对。若已有迁移虽然文件名不同、内容却与目标迁移完全相同，Updater 会保留原历史并跳过重复文件。构建生成文件由安装、开发、构建与检查命令重新生成，不参与主题校准。
